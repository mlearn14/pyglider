...
def get_profiles_new(ds, min_dp=10.0, filt_time=100, profile_min_time=300, _log=_log):
    ...
    _log : logging.Logger, default=_log
        Logger for messages.
    """
    ...
    if np.logical_or(
        len(good) == 0,
        np.nanmax(ds.pressure.values) - np.nanmin(ds.pressure.values) < min_dp,
    ):
        profile.fill(0)
        direction.fill(0)
        ...
        attrs = collections.OrderedDict(
            [
                ...
                (
                    "comment",
                    "Unique identifier of the profile, the mean profile timestamp",
                ),
                ("sources", "time pressure"),
        ...
        tm = ds.time.values.astype("timedelta64[s]") + np.datetime64(
            "1970-01-01T00:00:00"
        ...
        if mins[-1] < maxs[-1]:
            ...
            if good[-1] + 4 > len(ds.time):
                endmin = len(
                    ds.time
                )  # extend to the end of the time series if it is less than last valid pressure reading + 4
            else:
    ...
    attrs = collections.OrderedDict(
        [
            ...
            (
                "comment",
                "-1 = ascending, 0 = hovering or floating at surface, 1 = descending",
            ),
            ("sources", "time pressure"),
            ("method", "get_profiles_new"),
        ]
...
def check_profile_time_diff(
    ...
    ds (xr.Dataset)
        Dataset to filter
    gap_threshold (int)
        Maximum time gap (seconds), by default 30
    epochs (int)
    ...
    threshold_td = np.timedelta64(gap_threshold, "s")
    ...
    for i in range(epochs):
        ...
        _log.debug(
            f"Epoch {i}: {sum(bad)} samples removed. {sum(~bad)} samples remaining."
        )
